...
const SINGLETONS = [
  {id: 'home-en', _type: 'home', title: 'Home', prefix: 'home'},
  {id: 'settings-en', _type: 'settings', title: 'Settings', prefix: 'settings'},
]
const NEW_LANGUAGES = [
  {id: 'de', title: 'German'},
  {id: 'fr', title: 'French'},
...
async function createSingletons() {
  const documents = await Promise.all(
    SINGLETONS.map(async (singleton) => {
      ...
      const translations = NEW_LANGUAGES.map((language) => ({
        ...currentDoc,
        _id: `${singleton.prefix}-${language.id}`,
        _type: singleton._type,
        language: language.id,
      }))

      const metadata = {
        _id: `${singleton.prefix}-translation-metadata`,
